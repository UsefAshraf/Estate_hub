...
const CreateProperty = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        Swal.fire({
          ...
          text: `Latitude: ${position.coords.latitude.toFixed(6)}, Longitude: ${position.coords.longitude.toFixed(6)}`,
          timer: 2000,
  ...
  const handleGeocodeAddress = async () => {
    if (!propertyForm.address) {
      Swal.fire("Error", "Please enter an address first", "warning");
      return;
    }
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        Swal.fire({
          icon: "success",
          title: "Location Found!",
          text: `Coordinates set for: ${propertyForm.address}`,
          timer: 2000,
          showConfirmButton: false,
        });
      } else {
      ...
      Swal.fire(
        "Error",
        "Failed to geocode address. Please enter coordinates manually.",
        "error"
      );
    } finally {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (!propertyForm.latitude || !propertyForm.longitude) {
        Swal.fire("Error", "Please provide property location coordinates", "error");
        return;
      }

      // Validate coordinates
      const lat = parseFloat(propertyForm.latitude);
      const lon = parseFloat(propertyForm.longitude);

      if (isNaN(lat) || isNaN(lon) || lat < -90 || lat > 90 || lon < -180 || lon > 180) {
        Swal.fire("Error", "Invalid coordinates. Latitude must be between -90 and 90, Longitude between -180 and 180", "error");
        return;
      }
      ...
      // Build CreatePropertyRequest object with location
      const data: CreatePropertyRequest = {
        title: propertyForm.title,
        description: propertyForm.description,
        type: propertyForm.type as any,
          ...
          <div className="space-y-10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2">
                  ...
                  <input
                    ...
                    value={propertyForm.address}
                    onChange={handleInputChange}
                    placeholder="e.g., 123 Palm Avenue, Los Angeles, CA"
                    className="block w-full px-4 py-3 border border-gray-200 rounded-lg bg-gray-50 text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#C19A6B] focus:border-transparent transition-all"
                  />
                </div>
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900 mb-4 flex items-center">
                <MapPin className="w-6 h-6 mr-2 text-[#C19A6B]" />
                Property Location
              </h2>
              <div className="bg-[#FDFBF7] border border-[#E8DCC6] rounded-lg p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-2">
                      Latitude *
                    ...
                    <input
                      ...
                      className="block w-full px-4 py-3 border border-gray-200 rounded-lg bg-white text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#C19A6B] focus:border-transparent transition-all"
                    />
                ...
                <div className="mt-4 flex flex-wrap gap-3">
                  ...
                  <button
                    ...
                    disabled={!propertyForm.address || isGeocodingAddress}
                    className="flex items-center space-x-2 px-4 py-2 bg-[#E6D5B8] text-[#4A3B2A] rounded-lg hover:bg-[#C19A6B] hover:text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <MapIcon className="w-4 h-4" />
                    <span className="text-sm font-medium">
                      {isGeocodingAddress ? "Finding..." : "Get Coordinates from Address"}
                    </span>
                  </button>

                  {propertyForm.latitude && propertyForm.longitude && (
                    <a
                      href={`https://www.google.com/maps?q=${propertyForm.latitude},${propertyForm.longitude}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center space-x-2 px-4 py-2 bg-blue-50 text-blue-700 rounded-lg hover:bg-blue-100 transition-colors"
                    >
                      <MapPin className="w-4 h-4" />
                      <span className="text-sm font-medium">Preview on Map</span>
                    </a>
                  )}
                </div>
                ...
                <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                  <p className="text-sm text-blue-800">
                    <strong>Tip:</strong> Enter the address above, then click "Get Coordinates from Address" 
                    to automatically fill latitude and longitude. Or use "Use My Current Location" if you're 
                    at the property.
                  </p>
                </div>
              </div>
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full p-8 text-center transform transition-all scale-100 opacity-100">
            ...
            <p className="text-gray-600 mb-6">
              Your property listing has been successfully created with location coordinates.
            </p>
            <button
