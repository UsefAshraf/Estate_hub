...
const SearchResultsPage: React.FC = () => {
  ...
  const fetchProperties = async () => {
    ...
    try {
      const res = await getAllProperties({});
      if (res.data.success) {
        // ✅ Filter out sold properties immediately when fetching
        const availableProperties = res.data.data.filter(
  ...
  const applyFilters = useCallback(() => {
    ...
    // ✅ Double-check: Filter out sold properties (defensive programming)
    ...
    // Property status filter (rent/sale) - excluding "sold"
    if (propertyStatus !== "All") {
      filtered = filtered.filter(
        (property) => property.status.toLowerCase() === propertyStatus.toLowerCase()
      );
    ...
    setProperties(filtered);
  }, [allProperties, searchQuery, propertyType, propertyStatus, bedrooms, minPrice, maxPrice]);
  ...
  const handleClearFilters = () => {
    ...
    setMinPrice("");
    setMaxPrice("");
    // ✅ Reset to all available (non-sold) properties
    setProperties(allProperties);
    setShowFilters(false);
  };
  ...
  // ✅ Apply filters whenever searchQuery changes
  useEffect(() => {
    if (allProperties.length > 0) {
      applyFilters();
    }
