...
export const createProperty = (
  ...
  Object.entries(data).forEach(([key, value]) => {
    if (key === "images") {
      ...
      // Handle features array - stringify it
      formData.append("features", JSON.stringify(value));
    } else if (key === "location") {
      // CRITICAL FIX: Stringify location object as JSON
  ...
  if (hasImages) {
    ...
    Object.entries(data).forEach(([key, value]) => {
      if (key === "images" && value) {
        ...
        // Stringify features for FormData
        formData.append("features", JSON.stringify(value));
      } else if (key === "location" && value) {
        // Stringify location for FormData
        formData.append("location", JSON.stringify(value));
      } else if (value !== undefined && value !== null) {
        formData.append(key, String(value));
      }
    ...
    Object.entries(data).forEach(([key, value]) => {
      if (key === "features" && value) {
        ...
        // Location stays as object in JSON mode
...
export const markPropertyAsSold = (id: string) => {
  return API.patch(`/api/properties/${id}/sold`);
};
